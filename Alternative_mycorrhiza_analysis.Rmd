---
title: "Alternative_mycorrhiza_analysis"
...
#only keep AM and EM
myco_tsv <- subset(myco_tsv, (myco_tsv$myco_state == "arbuscular" | myco_tsv$myco_state == "ecto"))

#create new column and add binary values to column based on names 
...
```{r define formula}
# Formula 3. With ... (no alpha warning AIC 1037,3)
myco_formula_

# Formula 4. With ... (alpha warning, AIC 1026,5) 
myco_formula_
```
...
#construct model
myco_best_model <- mycorrhiza ~ 1 + Aspect + BulkDensity + PETWettestQuarter + OrganicCarbon + bio15 + bio18
...
model_myco <- phyloglm(formula = myco_best_model, data = GIS_myco, phy = tree_myco, 
...
summary(model_myco)
```
Conclusion: Both these formulas can be used, they result in the same final model. 
