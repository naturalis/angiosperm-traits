...
#to make sure) AND is not in the crops_names list -> add it to a new vector
...
Only the combinations that result in the best models are shown, see VIF_analysis_crops" for all 
...
```{r define formula}
# Formula 4a. With BIO4
crops_formula_4a <- domestication ~ Aspect + BulkDensity + ClayPercentage + minTempWarmest 
...
crops_best_model <- domestication ~ 1 + Aspect + ClayPercentage + PETWarmestQuarter + PETWettestQuarter

#fill in the arguments for the phyloglm function and execute it
model_crops <- phyloglm(formula = crops_best_model, data = GIS_crops, phy = tree_crops, 
...
#fill in the arguments for the phyloglm function and execute it
model_crops2 <- phyloglm(formula = crops_best_model2, data = GIS_crops, phy = tree_crops, 
                         method = "logistic_MPLE", btol = 100, log.alpha.bound = 8)
...
model_check_crops <- glm(domestication ~ 1 + ClayPercentage + PETWarmestQuarter, 
                         data = GIS_crops, family = binomial())
...
The results are plotted in the form of boxplots that show the differences between the variable
