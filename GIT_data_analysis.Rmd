...
combined_woody <- merge(raw_means_woody, GIS_woody, by = 1)
```

#### monthCountByTemp10
```{r}
#define two variables to plot
monthCountByTemp10_OMI <- combined_woody$monthCountByTemp10.x
monthCountByTemp10_RAW <- combined_woody$monthCountByTemp10.y
...
PETWettestQuarter_OMI <- combined_woody$PETWettestQuarter.x
PETWettestQuarter_RAW <- combined_woody$PETWettestQuarter.y
...
```{r}
#define two variables
OrganicCarbon_OMI <- combined_woody$OrganicCarbon.x
OrganicCarbon_RAW <- combined_woody$OrganicCarbon.y
...
```{r}
#define two variables
PhCaCL_OMI <- combined_woody$PhCaCL.x
PhCaCL_RAW <- combined_woody$PhCaCL.y
...
```{r}
#define two variables
Slope_OMI <- combined_woody$Slope.x
Slope_RAW <- combined_woody$Slope.y
...
```{r}
#define two variables
bio5_OMI <- combined_woody$bio5.x
bio5_RAW <- combined_woody$bio5.y
...
combined_myco <- merge(raw_means_myco, GIS_myco, by = 1)
```

#### ClayPercentage
```{r}
#define two variables
ClayPercentage_OMI <- combined_myco$ClayPercentage.x
ClayPercentage_RAW_myco <- combined_myco$ClayPercentage.y
...
```{r}
#define two variables
bio2_OMI <- combined_myco$bio2.x
bio2_RAW <- combined_myco$bio2.y
...
```{r}
#define two variables
bio15_OMI <- combined_myco$bio15.x
bio15_RAW <- combined_myco$bio15.y
...
combined_crops <- merge(raw_means_crops, GIS_crops, by = 1)
```

#### ClayPercentage
```{r}
#define two variables
ClayPercentage_OMI <- combined_crops$ClayPercentage.x
ClayPercentage_RAW_crops <- combined_crops$ClayPercentage.y
...
```{r}
#define two variables
PETWarmestQuarter_OMI <- combined_crops$PETWarmestQuarter.x
PETWarmestQuarter_RAW <- combined_crops$PETWarmestQuarter.y
