...
#load tsv file and make selection
swo_tsv <- read.table(file = "C:/Users/Esther/Documents/Master/Stage/sdmdl-angiosperm-data/data/woody.tsv", sep = '\t', header = TRUE)
swo_tsv_bin <- subset(swo_tsv, (swo_tsv$woody == 0 | swo_tsv$woody ==1))

#the taxa that should not be included (with value 2)
swo_tsv_2 <- subset(swo_tsv, (swo_tsv$woody == 2)) 
unwanted_tips <- as.character(unlist(c(swo_tsv_2[1])))
unwanted_numbers <- as.numeric(row.names(swo_tsv_2))

#make a subset of nex file with these taxa as condition and give new name 
swo_bin <- drop.tip(swo, unwanted_numbers)
